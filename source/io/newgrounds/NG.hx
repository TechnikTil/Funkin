...
class NG extends NGLite
{
  ...
  {
    if (_session == null || _session.status != SessionStatus.REQUEST_LOGIN) return null;
  ...
  inline function get_onMedalsLoaded()
    return medals.onLoad;

  @:deprecated("Use scoreBoards.onLoad")
  public var onScoreBoardsLoaded(get, never):Dispatcher;

  inline function get_onScoreBoardsLoaded()
    return scoreBoards.onLoad;

  @:deprecated("Use saveSlots.onLoad")
  public var onSaveSlotsLoaded(get, never):Dispatcher;

  inline function get_onSaveSlotsLoaded()
    return saveSlots.onLoad;
  ...
  **/
  public function new(appId = "test", ?sessionId:String, debug = false, ?callback:(LoginOutcome) -> Void)
  {
    host = getHost();
  ...
  **/
  static public function create(appId = "test", ?sessionId:String, debug = false, ?callback:(LoginOutcome) -> Void):Void
  {
   ...
   */
  static public function createAndCheckSession(appId = "test", debug = false, ?backupSession:String, ?callback:(LoginOutcome) -> Void):Void
  {
    var session = NGLite.getSessionId();
    if (session == null) session = backupSession;

    create(appId, session, debug, callback);

    if (core.sessionId != null) core.attemptingLogin = true;
  }
  ...
  override function checkInitialSession(callback:Null<(LoginOutcome) -> Void>, outcome:CallOutcome<SessionData>):Void
  {
   ...
   *                          popup blocker.
   */
  public function requestLogin(callback:(LoginOutcome) -> Void = null, passportHandler:String->Void = null):Void
  {
    if (attemptingLogin)
    {
    ...
    calls.app.startSession(true).addOutcomeHandler(onSessionReceive.bind(_, callback, passportHandler)).send();
  }

  function onSessionReceive(outcome:CallOutcome<SessionData>, callback:Null<(LoginOutcome) -> Void>, passportHandler:String->Void):Void
  {
    switch (outcome)
    {
      case SUCCESS(data):
      ...
      case FAIL(error):
        ...
        if (callback != null) callback(FAIL(ERROR(error)));
        return;
    }
    ...
    if (_session.status == REQUEST_LOGIN)
    {
      ...
      if (passportHandler != null) passportHandler(passportUrl);
      else
        openPassportUrl();
    }
    else
  ...
  function openPassportHelper(url:String):Void
  {
    ...
    #elseif mobile
    extension.webviewcore.WebView.openWithURL(url);
    #elseif sys
    switch Sys.systemName()
    {
      ...
      case name:
        logError("Unhandled systemName: " + name);
    }
    #else
  ...
  function checkSessionStatus(callback:Null<(LoginOutcome) -> Void>)
  {
    ...
    switch (_session.status)
    {
      ...
      case REQUEST_LOGIN:
        var call = calls.app.checkSession().addOutcomeHandler(checkSession.bind(_, callback));

        // Wait 3 seconds and try again
        timer(3.0, function():Void {
          logVerbose("3s elapsed, checking session again");

          // Check if cancelLoginRequest was called
          if (!_loginCancelled) call.send();
          else
          {
            log("login cancelled via cancelLoginRequest");
            endLoginAndCall(callback, FAIL(CANCELLED(MANUAL)));
  ...
  {
    var call = calls.app.endSession().addSuccessHandler(onLogOutSuccessful);

    if (onComplete != null) call.addOutcomeHandler((o) -> onComplete(o.toUntyped()));

    call.addSuccessHandler(onLogOut.dispatch).send();
  }
  ...
  **/
  ...
  {
    calls.gateway.getDatetime().addOutcomeHandler((outcome) -> switch (outcome)
    {
  ...
  public function requestServerTime(callback:(TypedOutcome<Date, CallError>) -> Void, useServerTime = false)
  {
    calls.gateway.getDatetime().addOutcomeHandler((outcome) -> switch (outcome)
    {
      case FAIL(error): callback(FAIL(error));
      case SUCCESS(data):
        if (useServerTime) callback(SUCCESS(data.getServerDate())); else callback(SUCCESS(data.getDate()));
    }).send();
  }
  ...
  static public function getHost():String
  {
    ...
    if (url == null || url == "") return "AppView";

    if (url.indexOf("file") == 0 || url.indexOf("127.0.0.1") != -1) return "LocalHost";

    if (urlParser.match(url)) return urlParser.matched(1);
