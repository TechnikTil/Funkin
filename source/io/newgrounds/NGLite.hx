...
class NGLite
{
  ...
  **/
  public function new(appId = "test", ?sessionId:String, debug = false, ?callback:(LoginOutcome) -> Void)
  {
    ...
    {
      calls.app.checkSession().addOutcomeHandler(checkInitialSession.bind(callback)).send();
    }
  }
  ...
  function checkInitialSession(callback:Null<(LoginOutcome) -> Void>, outcome:CallOutcome<SessionData>):Void
  {
    switch (outcome)
    {
      ...
      case FAIL(error):
        initialSessionFail(callback, error);
    }
  }
  ...
  **/
  static public function create(appId = "test", sessionId:String = null, ?callback:(LoginOutcome) -> Void):Void
  {
  ...
  **/
  static public function createAndCheckSession(appId = "test", backupSessionId:String = null, ?callback:(LoginOutcome) -> Void):Void
  {
    var session = getSessionId();
    if (session == null) session = backupSessionId;
  ...
  inline static public function getUrl():String
  {
    ...
    #elseif flash
    return flash.Lib.current.stage.loaderInfo != null ? flash.Lib.current.stage.loaderInfo.url : null;
    #else
    return null;
    #end
  }
  ...
  static public function getSessionId():String
  {
    ...
    if (index != -1)
    {
      ...
      {
        index = param.indexOf("=");
        if (index != -1 && param.substr(0, index) == "ngio_session_id") return param.substr(index + 1);
      }
    }
    #elseif flash
    if (flash.Lib.current.stage.loaderInfo != null
      && Reflect.hasField(flash.Lib.current.stage.loaderInfo.parameters,
        "ngio_session_id")) return Reflect.field(flash.Lib.current.stage.loaderInfo.parameters, "ngio_session_id");
    #end
  ...
  {
    if (_pendingCalls.length == 0 && _queuedCalls.length > 0) _queuedCalls.shift().send();
  }
  ...
  dynamic public function log(any:Dynamic, ?pos:PosInfos):Void
  { // TODO: limit access via @:allow

    haxe.Log.trace('[Newgrounds API] :: ${any}', pos);
  }

  /** used internally, logs if verbose is true */
  inline public function logVerbose(any:Dynamic, ?pos:PosInfos):Void
  { // TODO: limit access via @:allow

    if (verbose) log(any, pos);
  }

  /** Used internally. Logs by default, set this to your preferred error handling method */
  dynamic public function logError(any:Dynamic, ?pos:PosInfos):Void
  { // TODO: limit access via @:allow
  ...
  inline public function assert(condition:Bool, msg:Dynamic, ?pos:PosInfos):Bool
  { // TODO: limit access via @:allow
    if (!condition) logError(msg, pos);
  ...
  public function setupEncryption(key:String, cipher:Cipher = AES_128, format:EncodingFormat = BASE_64)
  {
